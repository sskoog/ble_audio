#!/usr/bin/env python3
"""
pack_benchmark_clips_4mb.py
Prepares 5.0s 16-bit mono PCM clips from 'data/10s clips/10s 48 kHz mono CLIP Monster.wav'
for sample rates: 48000, 44100, 32000, 24000, 16000, 8000 Hz.
Packages them into benchmark_clips_4mb.bin with an indexed header for 4MB flash partition.
"""
...
OUTPUT_BIN = r"data/benchmark_clips_4mb.bin"
...
def main():
    ...
    for sr in SAMPLE_RATES:
        temp_wav = f"data/temp_{sr}_5s.wav"
        print(f"Generating {sr} Hz 16-bit mono {DURATION_SEC}s clip...")
        cmd = [
            "ffmpeg", "-y", "-i", SOURCE_WAV,
            "-ar", str(sr), "-ac", "1", "-c:a", "pcm_s16le",
            "-t", str(DURATION_SEC), temp_wav
        ]
