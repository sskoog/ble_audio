...
// Standard 8x16 ASCII Font Bitmap Table (ASCII 32..127)
...
void LcdConsole::setBacklight(bool enable) {
    setBacklightBrightness(enable ? 20 : 0);
}

void LcdConsole::setBacklightBrightness(uint8_t percent) {
...
void LcdConsole::clear(uint16_t bg_color) {
...
void LcdConsole::drawChar(int x, int y, char c, uint16_t color, uint16_t bg_color) {
...
void LcdConsole::drawString(int x, int y, const char* str, uint16_t color, uint16_t bg_color) {
    if (!str) return;
    int cur_x = x;
    while (*str) {
...
void LcdConsole::drawHeader(const char* title) {
    if (!m_framebuffer) return;
...
void LcdConsole::printLine(int row, const char* text, uint16_t color, uint16_t bg_color) {
...
void LcdConsole::setRgbColor(uint8_t red, uint8_t green, uint8_t blue) {
